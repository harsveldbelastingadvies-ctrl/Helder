...
function HelpCenter({ onCreateInvoice, onCreateCustomer, onCreateExpense, onViewInvoices, onViewCustomers, onViewExpenses, onViewProfit, onViewVat, onViewYearEnd, onViewSettings }: { onCreateInvoice: () => void; onCreateCustomer: () => void; onCreateExpense: () => void; onViewInvoices: () => void; onViewCustomers: () => void; onViewExpenses: () => void; onViewProfit: () => void; onViewVat: () => void; onViewYearEnd: () => void; onViewSettings: () => void }) {
  ...
  const subscriptionItems = [
    { title: "Proefperiode", body: "Je start rustig met 14 dagen proberen. Daarna vraagt Helder om activatie, zodat het platform niet ongemerkt gratis doorloopt." },
    { title: "Betaling", body: "Betalen loopt via Mollie. Na een geslaagde betaling zet Helder je pakket automatisch op actief." },
  ...
  const supportItems = [
    { title: "Vraag over Helder", body: "Gebruik de hulpteksten in Helder of neem contact op als iets onduidelijk is. Liever één vraag te veel dan blijven zoeken." },
    { title: "Sparren over administratie", body: "Voor inhoudelijke administratievragen kun je met Ralf sparren. Helder helpt met overzicht; persoonlijk advies blijft maatwerk." },
...
function SupportCard() {
  return <section className="card support-card"><p className="eyebrow">CONTACT</p><h2>Hulp of sparren?</h2><p>Loop je vast in Helder of twijfel je over een administratieve keuze? Neem contact op met Ralf. Dan kijken we samen wat praktisch en verstandig is.</p><div className="support-list"><span><strong>Technische vraag</strong><small>Beschrijf wat je wilde doen en welke melding je zag.</small></span><span><strong>Administratievraag</strong><small>Gebruik Helder voor overzicht; inhoudelijk advies blijft persoonlijk maatwerk.</small></span></div><small>Reageer nooit met wachtwoorden, API-sleutels of volledige betaalgegevens in een bericht.</small></section>;
}
