...
type User = { id: string; name: string; email: string; companyName: string; emailVerified: boolean };
...
function InvoiceBrand({ logo, variant = "paper" }: { logo?: string; variant?: "paper" | "preview" }) {
  ...
  return variant === "preview"
    ? <div className="preview-brand"><span className="brand-mark">h</span><strong>helder</strong></div>
    : <div className="brand"><span className="brand-mark">h</span><span>helder</span></div>;
}
...
function LoadingScreen() {
  return <main className="auth-shell"><div className="auth-brand"><span className="brand-mark">h</span><span>helder</span></div><div className="loading-card"><span className="loading-spinner"/><p>Je administratie wordt klaargezet…</p></div></main>;
}
  ...
  return (
    <div className="welcome-backdrop" role="dialog" aria-modal="true" aria-labelledby="welcome-title">
      <section className="welcome-modal">
        ...
        <div className="welcome-mark"><span className="brand-mark">h</span></div>
        <p className="eyebrow">PERSOONLIJK WELKOM</p>
        <h1 id="welcome-title">Welkom bij Helder, {firstName}.</h1>
        <p className="welcome-message">
          Wat fijn dat je er bent. Ik wens je veel succes met je onderneming. Gebruik Helder om rust en overzicht te houden in je facturen, kosten, btw en klantcontact.
        </p>
...
function LoginScreen({ onLogin, onRegistered }: { onLogin: (email: string, password: string) => Promise<void>; onRegistered: () => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      if (mode === "recover" && !recoveryRequested) {
        ...
        const response = await fetch("/api/auth/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, companyName, email, password }) });
        const data = await response.json() as { error?: string };
  ...
  if (mode === "recover") {
    return <main className="auth-shell"><div className="auth-brand"><span className="brand-mark">h</span><span>helder</span></div><section className="auth-card"><p className="eyebrow">WACHTWOORDHERSTEL</p><h1>Nieuw wachtwoord instellen.</h1><p className="auth-intro">Vul je e-mailadres in. Lokaal toont Helder de code direct; online ontvang je de code per e-mail.</p><form onSubmit={submit}><label><span>E-mailadres</span><input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label>{recoveryCode && <p className="local-code">Herstelcode: <strong>{recoveryCode}</strong></p>}{recoveryRequested && <label><span>Herstelcode</span><input required value={enteredRecoveryCode} onChange={(event) => setEnteredRecoveryCode(event.target.value)} /></label>}{recoveryRequested && <label><span>Nieuw wachtwoord</span><input required type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>}{error && <p className="auth-error">{error}</p>}{recoveryMessage && <p className="security-success">{recoveryMessage}</p>}<button className="primary-button auth-submit" disabled={submitting}>{submitting ? "Even verwerken…" : recoveryRequested ? "Wachtwoord wijzigen" : "Herstelcode maken"}</button></form><button className="auth-link-button" onClick={() => { setMode("login"); setError(""); setRecoveryRequested(false); }}>Terug naar inloggen</button></section><p className="auth-footnote">Je gegevens blijven tijdens het bouwen alleen op deze computer.</p></main>;
  }

  return <main className="auth-shell commercial-auth-shell"><section className="commercial-auth-grid"><div className="commercial-panel"><div className="auth-brand commercial-brand"><span className="brand-mark">h</span><span>helder</span></div><p className="eyebrow">ADMINISTRATIE VOOR KLEINE ONDERNEMERS</p><h1>Rust in facturen, kosten, btw en klantcontact.</h1><p className="commercial-lead">Helder helpt ondernemers hun administratie bijhouden zonder boekhoudtaal. Je maakt facturen, voert kosten inclusief btw in, ziet je btw-overzicht en houdt klanten netjes bij.</p><div className="commercial-actions"><button className="primary-button" type="button" onClick={() => { setMode("register"); setError(""); }}>Start met Helder</button><button className="secondary-button" type="button" onClick={() => { setMode("login"); setError(""); }}>Ik heb al een account</button></div><div className="commercial-proof"><span><strong>Facturen</strong>PDF, logo en betaaltermijn</span><span><strong>Btw</strong>Te betalen of terug te krijgen</span><span><strong>CRM</strong>Notities en opvolgacties</span></div><div className="commercial-feature-grid"><article><Icon name="file" size={17}/><strong>Factureren zonder zoekwerk</strong><p>Klantgegevens, btw en vaste factuurtekst staan klaar zodra je een factuur maakt.</p></article><article><Icon name="receipt" size={17}/><strong>Kosten inclusief btw invoeren</strong><p>Ondernemers vullen het totaalbedrag in; Helder rekent de btw en kosten uit.</p></article><article><Icon name="percent" size={17}/><strong>Btw-overzicht in gewone taal</strong><p>Eerst het bedrag, daarna pas de uitleg hoe dit bedrag is opgebouwd.</p></article><article><Icon name="users" size={17}/><strong>Klanten en opvolging bij elkaar</strong><p>CRM-notities en acties blijven gekoppeld aan de juiste klant.</p></article></div></div><aside className="commercial-auth-side"><section className="auth-card commercial-login-card"><p className="eyebrow">{mode === "login" ? "WELKOM TERUG" : "NIEUW ACCOUNT"}</p><h1>{mode === "login" ? "Log veilig in op je administratie." : "Start rustig met je eigen account."}</h1><p className="auth-intro">{mode === "login" ? "Gebruik hetzelfde e-mailadres als eerder. Dan staan je klanten, facturen en kosten weer klaar." : "Maak een account aan en bouw je administratie stap voor stap op."}</p><div className="auth-tabs"><button type="button" className={mode === "login" ? "active" : ""} onClick={() => { setMode("login"); setPassword(""); setError(""); }}>Inloggen</button><button type="button" className={mode === "register" ? "active" : ""} onClick={() => { setMode("register"); setEmail(""); setPassword(""); setError(""); }}>Account aanmaken</button></div><form onSubmit={submit}>{mode === "register" && <><label><span>Jouw naam</span><input required autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} /></label><label><span>Bedrijfsnaam</span><input required autoComplete="organization" value={companyName} onChange={(event) => setCompanyName(event.target.value)} /></label></>}<label><span>E-mailadres</span><input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label><span>Wachtwoord</span><input required type="password" autoComplete={mode === "register" ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} /></label>{mode === "login" && <button className="auth-link-button" type="button" onClick={() => { setMode("recover"); setError(""); setRecoveryMessage(""); }}>Wachtwoord vergeten?</button>}{mode === "register" && <p className="password-help">Gebruik minimaal 8 tekens. Je kunt je e-mailadres later in Instellingen bevestigen.</p>}{error && <p className="auth-error">{error}</p>}{recoveryMessage && <p className="security-success">{recoveryMessage}</p>}<button className="primary-button auth-submit" disabled={submitting}>{submitting ? "Even controleren…" : mode === "register" ? "Account aanmaken" : "Inloggen"}</button></form>{mode === "login" ? <div className="demo-hint"><Icon name="check" size={16}/><p><strong>Eigen account</strong>Je administratie blijft gekoppeld aan je eigen account.</p></div> : <div className="demo-hint"><Icon name="check" size={16}/><p><strong>Rustige start</strong>Begin met bedrijfsgegevens, daarna klanten, facturen en kosten.</p></div>}</section><section className="commercial-pricing-card"><p className="eyebrow">PAKKETTEN</p><div><strong>Basis</strong><span>Voor zzp en kleine ondernemers die zelf overzicht willen.</span></div><div className="highlight"><strong>DGA-voorbereiding</strong><span>Voor B.V./DGA-administraties met duidelijke aandachtspunten.</span></div><div><strong>Begeleiding</strong><span>Voor ondernemers die af en toe willen sparren of laten meekijken.</span></div></section><p className="auth-footnote">Je gegevens blijven gekoppeld aan je eigen account. <Link href="/privacy">Lees de privacyuitleg</Link>.</p></aside></section></main>;

  return <main className="auth-shell"><div className="auth-brand"><span className="brand-mark">h</span><span>helder</span></div><section className="auth-card"><p className="eyebrow">{mode === "login" ? "WELKOM TERUG" : "NIEUW ACCOUNT"}</p><h1>{mode === "login" ? "Je administratie, helder geregeld." : "Begin met je eigen Helder-account."}</h1><p className="auth-intro">{mode === "login" ? "Log in met hetzelfde e-mailadres. Dan zie je je eigen klanten, facturen en kosten weer terug." : "Maak één eigen account aan. Alles wat je daarna toevoegt blijft aan dat account gekoppeld."}</p><div className="auth-tabs"><button type="button" className={mode === "login" ? "active" : ""} onClick={() => { setMode("login"); setPassword(""); setError(""); }}>Inloggen</button><button type="button" className={mode === "register" ? "active" : ""} onClick={() => { setMode("register"); setEmail(""); setPassword(""); setError(""); }}>Account aanmaken</button></div><form onSubmit={submit}>{mode === "register" && <><label><span>Jouw naam</span><input required autoComplete="name" value={name} onChange={(event) => setName(event.target.value)} /></label><label><span>Bedrijfsnaam</span><input required autoComplete="organization" value={companyName} onChange={(event) => setCompanyName(event.target.value)} /></label></>}<label><span>E-mailadres</span><input required type="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label><span>Wachtwoord</span><input required type="password" autoComplete={mode === "register" ? "new-password" : "current-password"} value={password} onChange={(event) => setPassword(event.target.value)} /></label>{mode === "login" && <button className="auth-link-button" type="button" onClick={() => { setMode("recover"); setError(""); setRecoveryMessage(""); }}>Wachtwoord vergeten?</button>}{mode === "register" && <p className="password-help">Gebruik minimaal 8 tekens. Je kunt je e-mailadres later in Instellingen bevestigen.</p>}{error && <p className="auth-error">{error}</p>}{recoveryMessage && <p className="security-success">{recoveryMessage}</p>}<button className="primary-button auth-submit" disabled={submitting}>{submitting ? "Even controleren…" : mode === "register" ? "Account aanmaken" : "Inloggen"}</button></form>{mode === "login" ? <div className="demo-hint"><Icon name="check" size={16}/><p><strong>Eigen account</strong>Gebruik hetzelfde e-mailadres als waarmee je je klant en factuur hebt aangemaakt.</p></div> : <div className="demo-hint"><Icon name="check" size={16}/><p><strong>Nieuw account</strong>Maak alleen een nieuw account als je echt met een lege administratie wilt starten.</p></div>}</section><p className="auth-footnote">Je gegevens blijven gekoppeld aan je eigen account. <Link href="/privacy">Lees de privacyuitleg</Link>.</p></main>;
}
...
function Dashboard({ user, companySettings, invoices, customers, expenses, vat, profitLoss, tasks, onCreate, onCreateCustomer, onCreateExpense, onViewInvoices, onViewCustomers, onViewExpenses, onViewProfit, onViewVat, onViewSettings, onOpenCustomer, onCompleteTask }: { user: User; companySettings: CompanySettings | null; invoices: Invoice[]; customers: Customer[]; expenses: Expense[]; vat: VatSummary; profitLoss: ProfitLossSummary | null; tasks: DashboardTask[]; onCreate: () => void; onCreateCustomer: () => void; onCreateExpense: () => void; onViewInvoices: () => void; onViewCustomers: () => void; onViewExpenses: () => void; onViewProfit: () => void; onViewVat: () => void; onViewSettings: () => void; onOpenCustomer: (id: string) => void; onCompleteTask: (id: string) => Promise<void> }) {
  ...
  const nextAction: NextAction = !settingsReady
    ...
    : countedInvoices.length === 0
      ? { label: "LOGISCHE VOLGENDE STAP", title: "Maak je eerste factuur", description: "Helder zet de klantgegevens en btw-berekening overzichtelijk voor je klaar.", buttonLabel: "Factuur maken", action: onCreate }
      : overdueCount > 0
        ...
        : urgentTask
          ...
          : expenses.length === 0
            ? { label: "BTW SLIMMER MAKEN", title: "Voer je eerste zakelijke kosten in", description: "Zo ziet Helder ook welke btw je mogelijk mag terugvragen.", buttonLabel: "Kosten invoeren", action: onCreateExpense }
            : hasVatActivity
              ...
              : { label: "RUSTIG VERDER", title: "Je basis staat goed", description: "Blijf facturen, kosten en klantacties bijhouden. Helder rekent ondertussen met je mee.", buttonLabel: "Instellingen controleren", action: onViewSettings, tone: "success" };
  return <>
...
function DgaPreparationCard({ onSettings }: { onSettings: () => void }) {
  return <section className="card dga-card"><div><p className="eyebrow">B.V. / DGA VOORBEREIDING</p><h2>Helder ondersteunt nu de basisadministratie.</h2><p>Facturen, kosten, btw, klanten en rapporten kun je blijven gebruiken. DGA-specifieke onderdelen zoals salaris, rekening-courant, dividend en vennootschapsbelasting moeten apart worden gecontroleerd of begeleid.</p></div><button className="secondary-button" onClick={onSettings}>Instellingen bekijken</button></section>;
}
...
function TaxReserveCard({ vat, profitLoss, onViewProfit, onViewVat }: { vat: VatSummary; profitLoss: ProfitLossSummary | null; onViewProfit: () => void; onViewVat: () => void }) {
  ...
  return <section className="card reserve-card"><div className="reserve-main"><span className="reserve-icon">€</span><div><p className="eyebrow">ZET ALVAST APART</p><h2>{hasNumbers ? euro(totalReserveCents) : "Nog geen bedrag nodig"}</h2><p>{hasNumbers ? "Een rustige schatting voor je belastingpotje: btw plus een voorzichtige 30% van je winst. Zo kom je later minder snel voor verrassingen te staan." : "Zodra er omzet, kosten en btw in de app staan, geeft Helder hier een eenvoudig spaaradvies."}</p></div></div><div className="reserve-breakdown"><span><small>Btw</small><strong>{euro(vatReserveCents)}</strong></span><span><small>Winstbelasting, grof geschat</small><strong>{euro(estimatedIncomeTaxCents)}</strong></span><span><small>Totaal apart houden</small><strong>{euro(totalReserveCents)}</strong></span></div><div className="reserve-actions"><button className="text-button" onClick={onViewVat}>Btw bekijken <Icon name="arrow" size={15}/></button><button className="text-button" onClick={onViewProfit}>Winst bekijken <Icon name="arrow" size={15}/></button></div></section>;
}
...
function StartChecklist({ items, onSettings }: { items: ChecklistItem[]; onSettings: () => void }) {
  ...
  return <section className="card start-card"><div className="start-intro"><div><p className="eyebrow">STARTKLAAR</p><h2>Je administratie rustig opbouwen</h2><p>Begin met deze stappen. Helder zet je daarna automatisch steeds meer op de rails.</p></div><div className="start-score"><strong>{completed}/{items.length}</strong><span>geregeld</span><div className="progress-track"><span style={{ width: `${percentage}%` }}/></div></div></div><div className={nextItem ? "start-next-step" : "start-next-step start-next-done"}><span>{nextItem ? "Nu handig" : "Mooi geregeld"}</span><div><strong>{nextItem ? nextItem.title : "Je basis staat klaar"}</strong><small>{nextItem ? nextItem.description : "Blijf rustig facturen, kosten en klantacties bijhouden. Helder rekent automatisch met je mee."}</small></div><button className={nextItem ? "primary-button" : "secondary-button"} onClick={nextItem ? nextItem.action : onSettings}>{nextItem ? nextItem.actionLabel : "Instellingen controleren"}</button></div><div className="start-list">{items.map((item) => <div className={item.done ? "start-item done" : "start-item"} key={item.title}><span className="start-check"><Icon name="check" size={13}/></span><div><strong>{item.title}</strong><small>{item.description}</small></div><button className={item.done ? "text-button" : "secondary-button"} onClick={item.action}>{item.actionLabel}</button></div>)}</div><div className="start-footer"><Icon name="settings" size={16}/><span>Controleer ook je bedrijfsgegevens voordat je een factuur echt verstuurt.</span><button className="text-button" onClick={onSettings}>Instellingen openen</button></div></section>;
}
...
function HelpCenter({ onCreateInvoice, onCreateCustomer, onCreateExpense, onViewInvoices, onViewCustomers, onViewExpenses, onViewProfit, onViewVat, onViewYearEnd, onViewSettings }: { onCreateInvoice: () => void; onCreateCustomer: () => void; onCreateExpense: () => void; onViewInvoices: () => void; onViewCustomers: () => void; onViewExpenses: () => void; onViewProfit: () => void; onViewVat: () => void; onViewYearEnd: () => void; onViewSettings: () => void }) {
  const helpSteps = [
    ...
    { number: "4", title: "Voer kosten en bonnetjes in", body: "Geef het bedrag inclusief btw op. Helder rekent zelf uit welk deel kosten en welk deel btw is.", action: "Kosten invoeren", onClick: onCreateExpense },
  ];
  const explainCards = [
  ...
  return <><section className="page-heading"><div><p className="eyebrow">HULP & UITLEG</p><h1>Waar wil je mee aan de slag?</h1><p>Korte uitleg zonder boekhoudtaal. Kies wat je wilt doen; Helder brengt je naar de juiste plek.</p></div><button className="primary-button" onClick={onCreateInvoice}><Icon name="plus" size={18}/>Factuur maken</button></section><section className="help-hero card"><div><p className="eyebrow">SNEL BEGINNEN</p><h2>De simpelste volgorde voor je administratie</h2><p>Als je net start, hoef je niet alles tegelijk te begrijpen. Begin bovenaan en werk rustig naar beneden.</p></div><div className="help-quick-actions"><button className="secondary-button" onClick={onViewCustomers}>Klanten bekijken</button><button className="secondary-button" onClick={onViewExpenses}>Kosten bekijken</button><button className="secondary-button" onClick={onViewInvoices}>Facturen bekijken</button></div></section><section className="help-steps">{helpSteps.map((step) => <article className="card help-step" key={step.title}><span>{step.number}</span><div><h2>{step.title}</h2><p>{step.body}</p><button className="text-button" onClick={step.onClick}>{step.action} <Icon name="arrow" size={15}/></button></div></article>)}</section><section className="card help-dictionary"><div><p className="eyebrow">WOORDENLIJST</p><h2>Boekhoudwoorden in gewone taal</h2><p>Een paar termen die vaak terugkomen in Helder, kort uitgelegd.</p></div><div>{dictionary.map((item) => <article key={item.term}><strong>{item.term}</strong><span>{item.explanation}</span></article>)}</div></section><section className="help-grid">{explainCards.map((card) => <article className="card help-info" key={card.title}><p className="eyebrow">UITLEG</p><h2>{card.title}</h2><p>{card.body}</p><button className="secondary-button" onClick={card.onClick}>{card.action}</button></article>)}<article className="card help-info"><p className="eyebrow">RAPPORT</p><h2>Ondernemersrapport</h2><p>Download één PDF met omzet, kosten, winst, open facturen en btw. Handig voor jezelf of je boekhouder.</p><a className="secondary-button" href="/api/reports/entrepreneur/export" download>Rapport downloaden</a></article><article className="card help-info help-privacy"><p className="eyebrow">VERTROUWEN</p><h2>Privacy en gegevensbeheer</h2><p>In Instellingen lees je welke gegevens Helder bewaart, waarvoor ze worden gebruikt en hoe je zelf grip houdt met back-ups, wachtwoord en sessies.</p><button className="secondary-button" onClick={onViewSettings}>Privacy bekijken</button></article><article className="card help-info help-live"><p className="eyebrow">ONLINE</p><h2>Wanneer kunnen we live?</h2><p>Helder is geschikt voor lokaal testen en demonstreren. Voor echte klantgegevens komen eerst veilige hosting, e-mail, automatische back-ups en een privacy/security-check.</p><ol><li><strong>Besloten test:</strong> enkele dagen tot ongeveer 1 week.</li><li><strong>Echte ondernemersgegevens:</strong> meestal ongeveer 2 tot 4 weken.</li></ol><button className="secondary-button" onClick={onViewSettings}>Instellingen controleren</button></article></section></>;
}
...
function LiveReadinessCard() {
  ...
  async function loadReadiness(showLoading = true) {
    ...
    try {
      ...
      setReadiness({
        ...
        nextAction: "Open Instellingen opnieuw of herstart Helder en probeer de livegang-check nog een keer.",
        items: [],
      });
    } finally {
      setLoading(false);
  ...
  return <section className={readiness?.ok ? "card live-card live-card-ok" : "card live-card"}><p className="eyebrow">LIVEGANG-CHECK</p><div className="readiness-score"><strong>{loading ? "…" : `${score}%`}</strong><span>klaar</span></div><div className="progress-track"><span style={{ width: `${score}%` }}/></div><p>{loading ? "Helder controleert de belangrijkste livegangpunten." : readiness?.message}</p>{!loading && readiness?.nextAction && <div className="live-next-action"><strong>Volgende actie</strong><span>{readiness.nextAction}</span></div>}{openItems.length > 0 && <div className="live-items">{openItems.map((item) => <div key={item.key}><span>{item.ok ? "✓" : "!"}</span><p><strong>{item.label}</strong>{item.detail}<small>{item.action}</small></p></div>)}</div>}<button type="button" className="secondary-button" onClick={() => void loadReadiness(true)} disabled={loading}>{loading ? "Controleren…" : "Opnieuw controleren"}</button></section>;
}
...
function BackupCard() {
  ...
  async function restoreBackup(file: File | undefined) {
    ...
    try {
      ...
      setMessage("Back-up teruggezet. Helder wordt opnieuw geladen…");
      window.setTimeout(() => window.location.reload(), 900);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Kies een geldig Helder-back-upbestand.");
      setBusy(false);
    }
  }
...
function PrivacyCard() {
  return <section className="card privacy-card"><p className="eyebrow">PRIVACY</p><h2>Gegevensbeheer in gewone taal</h2><p>Helder bewaart alleen gegevens die nodig zijn om je administratie te gebruiken. In deze lokale bouwversie blijven je gegevens op deze computer.</p><div className="privacy-list"><div><strong>Wat bewaren we?</strong><span>Accountgegevens, bedrijfsgegevens, klanten, facturen, kosten, bonnetjes, notities en acties.</span></div><div><strong>Waarvoor?</strong><span>Om facturen te maken, btw te berekenen, kosten te bewaren en klantopvolging overzichtelijk te houden.</span></div><div><strong>Wat kun je zelf?</strong><span>Je gegevens aanpassen, een back-up downloaden, je wachtwoord wijzigen en overal uitloggen.</span></div><div><strong>Wat delen we nu?</strong><span>In deze lokale versie delen we niets met externe partijen. Bij online hosting moet dit opnieuw duidelijk worden vastgelegd.</span></div></div><Link className="secondary-button" href="/privacy" target="_blank">Privacyverklaring openen</Link><small>Concepttekst voor transparantie. Voor een live platform moet dit nog worden uitgewerkt tot een volledige privacyverklaring.</small></section>;
}
...
function YearEndChecklist({ invoices, expenses, vat, profitLoss, onViewInvoices, onCreateExpense, onViewExpenses, onViewProfit, onViewVat, onViewSettings }: { invoices: Invoice[]; expenses: Expense[]; vat: VatSummary; profitLoss: ProfitLossSummary | null; onViewInvoices: () => void; onCreateExpense: () => void; onViewExpenses: () => void; onViewProfit: () => void; onViewVat: () => void; onViewSettings: () => void }) {
  ...
  const items: ChecklistItem[] = [
    ...
    { title: vatHasNumbers ? "Btw-overzicht staat klaar" : "Btw-overzicht vullen", description: vatHasNumbers ? `${vat.period}: ${euro(Math.abs(vat.payableVatCents))} ${vat.payableVatCents >= 0 ? "te betalen" : "terug te krijgen"}.` : "Zodra facturen en kosten erin staan, rekent Helder dit voor je uit.", done: vatHasNumbers, actionLabel: "Btw bekijken", action: onViewVat },
...
function VatOverview({ vat, profitLoss, onViewProfit }: { vat: VatSummary; profitLoss: ProfitLossSummary | null; onViewProfit: () => void }) {
  ...
  return <>
    ...
    <section className={isPayable ? "vat-hero card" : "vat-hero vat-hero-refund card"}>
      <div><p className="eyebrow">BELANGRIJKSTE BEDRAG</p><h2>{resultTitle}: {resultAmount}</h2><p>{isPayable ? "Dit is de btw die je volgens Helder moet afdragen voor deze periode." : "Je hebt in deze periode meer btw betaald op kosten dan ontvangen op verkoopfacturen."} Dit blijft een concept tot je de aangifte controleert.</p></div>
