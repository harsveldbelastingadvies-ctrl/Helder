...
export function AdministrationApp() {
  ...
  async function deleteCustomer(customerId: string) {
    ...
    const data = await response.json() as { error?: string };
    if (!response.ok) throw new Error(data.error ?? "De klant kon niet worden verwijderd.");
    setCustomers((current) => current.filter((customer) => customer.id !== customerId));
    setTasks((current) => current.filter((task) => task.customerId !== customerId));
    showToast("Klantgegevens verwijderd");
  }
    ...
    <div className="app-shell">
      ...
      <main className="main-area">
        ...
        {editingInvoice ? (
          ...
          <CustomerDetail customer={customers.find((item) => item.id === selectedCustomerId)!} onBack={() => setSelectedCustomerId(null)} onEdit={(customer) => { setSelectedCustomerId(null); setCustomerEditor(customer); }} onDelete={deleteCustomer} onDeleted={() => { setSelectedCustomerId(null); setView("customers"); }} />
        ) : creating ? (
          ...
          <div className="page-content">
            ...
            {view === "customers" && <Customers customers={customers} onCreate={() => setCustomerEditor("new")} onEdit={setCustomerEditor} onOpen={setSelectedCustomerId} onDelete={deleteCustomer} />}
...
function Customers({ customers, onCreate, onEdit, onOpen, onDelete }: { customers: Customer[]; onCreate: () => void; onEdit: (customer: Customer) => void; onOpen: (id: string) => void; onDelete: (id: string) => Promise<void> }) {
  const [query, setQuery] = useState("");
