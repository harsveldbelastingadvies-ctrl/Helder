...
LOGFILE="/tmp/helder-server.log"

clear
echo "Helder wordt geopend..."
echo ""
...
if [ ! -f "node_modules/next/dist/bin/next" ]; then
  echo "Ik kan de projectbestanden van Helder niet compleet vinden."
  echo "Geef deze melding even door aan Codex."
  echo ""
  read "reply?Druk op Enter om dit venster te sluiten."
  exit 1
...
echo "Stap 1 van 3: controleren of Helder al aan staat."
STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 2 "http://127.0.0.1:3000/")
if [ "$STATUS" -ge 200 ] && [ "$STATUS" -lt 400 ]; then
  echo "Helder stond al aan."
...
if [ "$STATUS" != "000" ]; then
  echo "Helder reageerde niet goed. Ik stop de oude server en start schoon opnieuw."
...
echo ""
echo "Helder kon niet op tijd worden gestart."
